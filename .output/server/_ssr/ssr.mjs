...
async function getServerEntry() {
	if (!serverEntryPromise) serverEntryPromise = import("./server-CecSDaFP.mjs").then((n) => n.t).then((m) => m.default ?? m);
	return serverEntryPromise;
}
async function handleApiInterview(request) {
	try {
		const body = await request.json();
		const payloadJson = JSON.stringify(body);
		const rootDir = processModule.cwd().endsWith("frontend") ? path.resolve(processModule.cwd(), "..") : processModule.cwd();
		...
		return new Promise((resolve) => {
			const child = execFile("python3", [scriptPath, "-"], {
				...
				env: {
					...processModule.env,
					PYTHONPATH: `${aiAgentDir}:${processModule.env["PYTHONPATH"] || ""}`
				}
			}, (error, stdout, stderr) => {
				if (error) {
...
var server_default = { async fetch(request, env, ctx) {
	try {
		...
		if (request.method === "POST" && url.pathname === "/api/interview") return await handleApiInterview(request);
