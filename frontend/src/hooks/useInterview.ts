...
export function useInterview() {
  ...
  const start = useCallback(
    async (selected: Candidate) => {
      ...
      try {
        ...
        setError(
          e instanceof InterviewApiError ? e.message : "Unexpected error starting interview.",
        );
        setSessionId(null);
        setCandidate(null);
      } finally {
  ...
  const send = useCallback(
    async (message: string) => {
      ...
      const candidateMsgId = newId();
      setMessages((prev) => [
        ...prev,
        { id: candidateMsgId, role: "candidate", content: text, at: Date.now() },
      ]);
      setError(null);
      setPending(true);
      try {
        ...
        // If the turn returned an answer evaluation, attach it to candidate's message
        if (res.evaluation) {
          setMessages((prev) =>
            prev.map((m) => (m.id === candidateMsgId ? { ...m, evaluation: res.evaluation } : m)),
          );
        }
